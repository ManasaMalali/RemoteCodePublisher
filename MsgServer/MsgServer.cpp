...
// Author:      Manasa Malali Nagabhushana, Syracuse University        //
//              SUID:845368670, mmalalin@syr.edu                       //
/////////////////////////////////////////////////////////////////////////
/* Package Operations :
* ===================
...
*	void Download();
*	void SetAttributesForpublisher();
*
*  Maintanence History :
*--------------------
*  ver 1.0
...
void ClientHandler::SetAttributesForPublisher()
{
	CodeAnalysisExecutive CE;
	nargc = 6;
	std::string str = "..\\ServerRepository"; //+ category;
	...
	nargv[4] = "/f";
	nargv[5] = "/r";
	CE.Totest(nargc, nargv);
}

void ClientHandler::readAttribString(Socket& socket, HttpMessage& msg)
{
	while (true) {
		std::string attribString = socket.recvString('\n');
		if (attribString.size() < 2) break;
...
HttpMessage ClientHandler::readMessage(Socket& socket) {
	...
	if (msg.attributes()[0].first == "POST") {
		...
		else if (msg.attributes()[0].second == "PUBLISH") SetAttributesForPublisher(); 
		else if (msg.attributes()[0].second == "DOWNLOAD") Download();   
...
void ClientHandler::SetAttributes()
{	//std::string category = msg.findValue("Category");
	
	for (int i = 0; i<6; i++)
	newargv[i] = { 0 };
	newargc = 5;
	newargv[1] = "..\\ServerRepository\\";
	newargv[2] = "*.h";
	newargv[3] = "*.cpp";
	newargv[4] = "..\\ServerRepository\\Index.htm";
...
HttpMessage Server::makeMessage(size_t n, const std::string& body, const EndPoint& ep)
{
	...
	case 2:
		msg.clear();
		msg.addAttribute(HttpMessage::attribute("GET", "VIEW"));
		break;
	case 3:
		msg.clear();
...
void Server::execute(const size_t TimeBetweenMessages, const size_t NumMessages) {
	try {
		...
		std::vector<std::string>filescss = FileSystem::Directory::getFiles("../ServerRepository", "*.css");
		std::vector<std::string>filesjs = FileSystem::Directory::getFiles("../ServerRepository", "*.js");
