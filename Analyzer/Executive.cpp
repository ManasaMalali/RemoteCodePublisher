...
// Source:      Jim Fawcett, Syracuse University, CST 4-201        //
...
void CodeAnalysisExecutive::helpDisplay() {}

//Get the file list for the Executive
Files CodeAnalysisExecutive::GetFileList(int argc, char* argv[]) { 
	using namespace CodeAnalysis;
	Files FileList;
	CodeAnalysisExecutive exec;
	bool succeeded = exec.ProcessCommandLine(argc, argv);
	exec.getSourceFiles();
	FileList = exec.getAllsubFiles();
	return FileList;
}

//----< conduct code analysis >--------------------------------------
#include <fstream>
void CodeAnalysisExecutive::Totest(int argc, char* argv[]){
	using namespace CodeAnalysis;
	CodeAnalysisExecutive exec;
	bool succeeded = exec.ProcessCommandLine(argc, argv);
	exec.getSourceFiles();
	...
	Files sourcefiles = exec.getAllsubFiles();
	DepAnalyzer Analyzer;
	std::unordered_map<std::string, std::vector<std::string>> result = Analyzer.Analyze(sourcefiles);
	ExpandCollapse ec;
	std::unordered_map<std::string, std::vector<std::pair<int, int>>> MapFiles;
	MapFiles = ec.RetrieveFileLength();
	Publisher p1;
	p1.GetAllFiles(result, MapFiles);
	p1.HTMLMain();

	/*if (argc == 7)
	{
		p1.toHtml(Analyzer.DepAnal(exec.getAllsubFiles()), argv[6]);
	}
	else if (argc == 6)
	{
		p1.toHtml(Analyzer.DepAnal(exec.getAllsubFiles()), "");
	}
	p1.htmlLinks();*/
}
...
int main(int argc, char* argv[])
{
  ...
  std::cout << "\n-----------------------------------------------------------------------------------------------------------------------------------" << std::endl;
  std::cout << "\n-----------------------------------------------------------------------------------------------------------------------------------" << std::endl;
  std::cout << " Requirement 8 : Developed command line processing to define the files to publish by specifying path and file patterns." << std::endl;
  std::cout << "   - The command line accepts the filepaths, filepatterns (By default and automated Home page is launched,\n if the .htm file name is given in the command prompt then Both the homepage and the .htm page specified will be launched in different tabs " << std::endl;
  std::cout << "\n-----------------------------------------------------------------------------------------------------------------------------------" << std::endl;
  std::cout << "\n-----------------------------------------------------------------------------------------------------------------------------------" << std::endl;
  std::cout << " Requirement 9 : The CodePublisher is demonstarted on the Project 3 package itself hence publishing all important packages of Project3\n --- Publisher package " << std::endl;
  std::cout << " Below are the html links generated with Project 3 packages" << std::endl;
  p1.htmlLinks();
  std::cout << "\n-----------------------------------------------------------------------------------------------------------------------------------" << std::endl;
  std::cout << "\n-----------------------------------------------------------------------------------------------------------------------------------" << std::endl;
  std::cout << " Requiremnt 10 : Executive.cpp (Test Executive) is the automated unit test suite that demonstrates all requirements of this project" << std::endl;
  std::cout << "\n-----------------------------------------------------------------------------------------------------------------------------------" << std::endl;
  return 0;
}
