...
void ClientReceiver::argfunc(int argc, char* argv[])
{
	for (int i = 0; i<100; i++)
		newargv[i] = { 0 };
...
HttpMessage MsgClient::makeMessage(size_t n, const std::string& body, const EndPoint& ep)
{
	...
	case 3:
		msg.clear();
		msg.addAttribute(HttpMessage::attribute("POST", "VIEW"));
		break;
	case 4:
		msg.clear();
...
bool MsgClient::sendFile(const std::string& filename, Socket& socket)
{
	...
	std::string fqname = "../files/" + filename;
	...
	try {
		...
		for (size_t i = 1; i <= NumMessages; i++) {
			...
			if (msg.attributes()[0].second == "UPLOAD") {
				std::vector<std::string> files = FileSystem::Directory::getFiles("../files", "*.*");
...
void ClientReceiver::readAttribString(Socket& socket, HttpMessage& msg)
{
	while (true) {
		std::string attribString = socket.recvString('\n');
		if (attribString.size() < 2)
			break;
...
HttpMessage ClientReceiver::readMessage(Socket& socket) {
	...
	if (msg.attributes()[0].first == "GET") {
		...
		else if (msg.attributes()[0].second == "VIEW")
			{
				std::cout << "\n\tThe files in the selected category are listed below:" << std::endl;
				for (int i = 5; i < msg.attributes().size(); i++)
					std::cout << std::endl << "\t" << msg.attributes()[i].second << std::endl;					
			}
...
{
	std::string dirName = "../files/DownloadedFiles";
	_mkdir(dirName.c_str());
	std::string fqname = "../files/DownloadedFiles/" + filename;
